...
const AssistantModal: React.FC<AssistantModalProps> = ({ onClose }) => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      const aiResponse = await runQuery(prompt);
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="relative w-full h-[60vh] sm:h-[480px] sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col overflow-hidden"
